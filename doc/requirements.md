...
Stack 是一个 `action` 类型的移动端小游戏：Time the tap — 候选块在塔顶滑动，点屏幕落下，没对齐的部分切掉飞落。完美对齐能让塔重新长粗、combo 累积爆奖。

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #fff、#ffc555、#ff6ad5、rgba(255,255,255,0.08)、rgba(255,255,255,0.6)、rgba(255,255,255,0.45)、rgba(0,0,0,0.55)、#060716；高亮元素用于可点击目标、得分、结果或稀有状态。
...
- `BLOCK_DEPTH`：14
- `INITIAL_BLOCK_W`：240
- `CANDIDATE_VX_BASE`：260
- `CANDIDATE_VX_GROWTH`：3.2
- `CANDIDATE_VX_CAP`：540
- `PERFECT_TOLERANCE`：7
- `PERFECT_REGROWTH`：9
- `TOP_ANCHOR_FRAC`：0.42
- `CAMERA_LERP`：9
- `HUE_START`：0
- `DROP_OFFSET`：26
- `DROP_SPRING_K`：520
